...
/** Other manually turned off things include 
    -"Navigation Tabs" of the sections that are false below in both index.html and portfolio-details.html
    -"Address" and "Call Us" in contact section
    -"Birthday", "Website", "Age", "Degree" in About Section
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateProjectDetails(index) {
    ...
    // // Use innerHTML to allow <br> tags in the description
    ...
    if (audio) {
        if (audioSection) {
            ...
            const audioSource = audioSection.querySelector('audio source');
            if (audioSource) {
        ...
        if (audioSection) {
            audioSection.style.display = 'none'; // Hide audio if no audio source is present
        }
    }
} else if (audio) {
    if (audioSection) {
    ...
    if (embedSection) {
        embedSection.style.display = 'none'; // Hide video if no video source is present
    }
} else {
    if (audioSection) {
        audioSection.style.display = 'none'; // Hide audio if no audio source
    }
    if (embedSection) {
        embedSection.style.display = 'none'; // Hide video if no video source
    }
  }
  ...
  swiper.on('slideChange', () => {
    ...
    updateAudioPlayer(swiper.realIndex);
